...
GET /api/statistics/bulk?subject_ids=1,2,3
"""
...
def _empty_stats(subject_id, subject_title, online_exams_info):
    return {
...
def _finalize_stats(subject_id, subject_title, papers_details, attempts_details, online_exams_info):
    """Calcul commun (moyenne/médiane/écart-type/distribution) — partagé entre
    le endpoint par sujet et le endpoint groupé, sur des listes déjà filtrées
    et scopées à UN sujet chacune."""
    all_entries = papers_details + attempts_details
    if not all_entries:
        return _empty_stats(subject_id, subject_title, online_exams_info)
...
def get_subject_statistics(subject_id):
    try:
        ...
        session.close()
        return jsonify(_finalize_stats(subject_id, subject.title, papers_details, attempts_details, online_exams_info))
...
@statistics_bp.route('/api/statistics/bulk', methods=['GET'])
@paseto_required
def get_subjects_statistics_bulk():
    """Correctif montée en charge (29/08, audit) : la page Résultats
    professeur appelait GET /api/statistics/<id> une fois par sujet
    (Promise.all côté client — parallèle mais toujours N requêtes/N
    connexions DB). Ce endpoint groupé fait le même calcul par sujet, mais
    avec une seule requête DB par table (papers/exams/attempts) au lieu
    d'une par sujet, puis regroupe en mémoire — le résultat par sujet est
    identique à un appel individuel."""
    try:
        user_id = get_current_user_id()
        session = get_session()
        user    = session.query(User).filter_by(id=user_id).first()
        ...
        raw_ids = request.args.get('subject_ids', '')
        try:
            subject_ids = [int(x) for x in raw_ids.split(',') if x.strip()]
        except ValueError:
            session.close(); return jsonify({'error': 'subject_ids invalide'}), 400
        if not subject_ids:
            session.close(); return jsonify({})

        subjects = session.query(Subject).filter(Subject.id.in_(subject_ids)).all()
        if user.role == UserRole.PROFESSOR:
            subjects = [s for s in subjects if s.creator_id == user_id]
        subjects_by_id = {s.id: s for s in subjects}
        allowed_ids = list(subjects_by_id.keys())
        if not allowed_ids:
            session.close(); return jsonify({})

        papers = session.query(StudentPaper).options(
            joinedload(StudentPaper.student)
        ).filter(StudentPaper.subject_id.in_(allowed_ids)).all()
        ...
        online_exams = session.query(OnlineExam).filter(OnlineExam.subject_id.in_(allowed_ids)).all()
        exams_by_subject: dict = {}
        exam_to_subject: dict = {}
        for e in online_exams:
            exams_by_subject.setdefault(e.subject_id, []).append(e)
            exam_to_subject[e.id] = e.subject_id

        attempts_by_subject: dict = {}
        if exam_to_subject:
            for att in session.query(ExamAttempt).options(
                joinedload(ExamAttempt.student),
                joinedload(ExamAttempt.exam)
            ).filter(
                ExamAttempt.exam_id.in_(list(exam_to_subject.keys())),
                ExamAttempt.score.isnot(None)
            ).all():
                sid = exam_to_subject.get(att.exam_id)
                if sid is None:
                    continue
                attempts_by_subject.setdefault(sid, []).append({
                    'id':           att.id,
                    'student_name': att.student.full_name if att.student else 'Inconnu',
                    'student_email':att.student.email     if att.student else 'N/A',
                    'score':        att.score,
        ...
        for sid in allowed_ids:
            subj = subjects_by_id[sid]
            p_details = papers_by_subject.get(sid, [])
            a_details = attempts_by_subject.get(sid, [])
            online_exams_info = [{
                'id': e.id, 'title': e.title, 'status': e.status.value,
                'start_time': e.start_time.isoformat() if e.start_time else None,
                'attempts_count': sum(1 for a in a_details if a['exam_title'] == e.title),
            } for e in exams_by_subject.get(sid, [])]
            result[str(sid)] = _finalize_stats(sid, subj.title, p_details, a_details, online_exams_info)
        ...
        print(f"ERROR get_subjects_statistics_bulk: {e}")
        import traceback; traceback.print_exc()
        return jsonify({'error': str(e)}), 500
